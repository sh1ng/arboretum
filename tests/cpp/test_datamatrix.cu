...
TEST(DataMatrix, HistWideRange) {
  ...
  for (auto i = 0; i < rows; ++i) {
    printf("%u %u\n", unsigned(i),
           unsigned(data.GetHostData<unsigned char>(0)[i]));
...
TEST(DataMatrix, Hist255) {
  ...
  for (auto i = 0; i < rows; ++i) {
    if (i < 1020) {
      ASSERT_EQ(data.GetHostData<unsigned char>(0)[i], (i / (rows / hist_size)))
        << i;
    } else {
      ASSERT_EQ(data.GetHostData<unsigned char>(0)[i], 254) << i;
    }
  }
}
...
TEST(DataMatrix, HistSkewedRange) {
  ...
  ASSERT_EQ(data.GetHostData<unsigned char>(0)[6], 3);
...
TEST(DataMatrix, HistTinyRange2) {
  ...
  ASSERT_EQ(data.max_feature_size, 1);
  ASSERT_EQ(data.reduced_size[0], 1);
  ...
  ASSERT_EQ(data.GetHostData<unsigned char>(0)[4], 0);
  ASSERT_EQ(data.GetHostData<unsigned char>(0)[5], 0);
  ASSERT_EQ(data.GetHostData<unsigned char>(0)[6], 0);
  ASSERT_EQ(data.GetHostData<unsigned char>(0)[7], 0);
  ASSERT_EQ(data.GetHostData<unsigned char>(0)[8], 0);
  ASSERT_EQ(data.GetHostData<unsigned char>(0)[9], 0);
  ...
  ASSERT_FLOAT_EQ(data.data_reduced_mapping[0][0],
